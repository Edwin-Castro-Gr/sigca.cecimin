...
class C_procedimientos extends CI_Controller {
	...
	public function index() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) {
			...
			$data_usua['titulo']="Procedimientos CX";
			$data_usua['origen']="Cirugias";
			$data_usua['contenido']='procedimientoscx/index';
			$data_usua['entrada_js']='_js/procedimientoscx.js';
	...
	public function listar_tabla() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) {
			...
			if(!$this->input->is_ajax_request()) {
				...
				echo listar_procedimientoscx_tabla('WEB');
			}
		}
	}
	...
	public function guardar() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) 
			...
			if(!$this->input->is_ajax_request()) {
				...
				//echo "ingreso";
				$registro=array(
					'codigo_cx'=>$this->input->post('codigo_cx'), 
					'nombre'=>$this->input->post('nombre'), 
					'tiempo_cx'=>$this->input->post('tiempo_cx'), 
					'fecha_registro'=>date('Y-m-d H:i:s'), 
					'id_usuario'=>$this->session->userdata('C_id_usuario'), 
					'estado'=>'1'
				);

				$query = $this->general_model->insert('procedimientos_cx', $registro);
				if($query >= 1)
					echo '1';
				else {
					echo '<div class="alert alert-danger"><i class="fa fa-ban"></i><strong>¡Error!</strong><br>';
					switch($query) {
						case "1062": echo "El procedimiento ingresado, ya se encuentra registrado; Por favor verifique los datos!"; break;
	...
	public function pdf() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) 
	        ...
	        $pdf->hoja = 'LETTER';
	        $pdf->SetTitle("SIGCA - Listado de Procedimientos Quirúrgicos", true);
	        $pdf->SetLeftMargin(7);
            ...
            $pdf->Cell(0,0,utf8_decode('LISTADO GENERAL DE PROCEDIMIENTOS  '), 0, 0, 'C', false);
            $pdf->Ln(10);
			...
			$query = $this->general_model->consulta_personalizada($campos, 'procedimientos_cx', '', '', 0, 0);
			...
			$pdf->Cell(10,5,utf8_decode("CODIGO"),'LTRB',0,'C',$fill);
			$pdf->Cell(50,5,utf8_decode("NOMBRE"),'LTRB',0,'C',$fill);
			$pdf->Cell(10,5,utf8_decode("TIEMPO CIRUGIA"),'LTRB',0,'C',$fill);
	        ...
	        foreach ($encabezados as $row) {
                ...
                $pdf->Cell(10,5,utf8_decode($row->Codigo),'LTRB',0,'C',$fill);
                $pdf->Cell(50,5,utf8_decode($row->Nombre),'LTRB',0,'C',$fill);
                $pdf->Cell(10,5,utf8_decode($row->Tiempo),'LTRB',0,'C',$fill);
	        ...
	        $pdf->Output('I', "Listado_Procedimientos_Cx.pdf");
	...
	public function excel() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) 
			...
			$filename = "Listado_Procedimientos_Cx.xls";
		    header ("Content-Disposition: attachment; filename=".$filename ); 
			header ("Content-Type: application/vnd.ms-excel");
			
			$this->load->helper('funciones_tabla');
			
		    echo utf8_decode('<table border="1"><tr><th colspan="7">LISTADO GENERAL DE PROCEDIMIENTOS CX</th></tr></table><br>');

		    echo '<table border="1">';
            echo utf8_decode(listar_procedimientoscx_tabla('EXCEL')); 
            echo '</table>';			
		}//-Valida Inicio de Session
	}
	...
	public function inactivar() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) 
			...
			if(!$this->input->is_ajax_request()) {
				...
				$query = $this->general_model->update('procedimientos_cx', 'id_procedimiento', $this->input->post('idreg'), $registro);
				if($query=="OK")
					echo '1';
				else {
	...
	public function modificar() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) 
			...
			$query=$this->general_model->select_where('id_procedimiento, codigo_cx, nombre, tiempo_cx, estado', 'procedimientos_cx', array('id_procedimiento' => $id) );
			$row = $query->row_array();
				
			$arr['procedimientos_cx'] = array('id_procedimiento'=>$row['id_procedimiento'], 'codigo_cx'=>$row['codigo_cx'],'nombre'=>$row['nombre'],'tiempo_cx'=>$row['tiempo_cx'], 'estado'=>$row['estado']);
	...
	public function actualizar() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) 
			...
			if(!$this->input->is_ajax_request()) {
				...
				$query = $this->general_model->update('procedimientos_cx', 'id_procedimiento', $this->input->post('idregistro'), $registro);
				if($query=="OK")
					echo '1';
				else {
					echo '<div class="alert alert-danger"><i class="fa fa-ban"></i><strong>¡Error!</strong><br>';
	...
	public function ver_registro() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) {
			...
			if(!$this->input->is_ajax_request()) {
		    	...
		    	$query = $this->general_model->consulta_personalizada($campos, 'procedimientos_cx', ' id_procedimiento = "'.$idreg.'" ', '', 0, 0);
