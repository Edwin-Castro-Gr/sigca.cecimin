...
class D_conceptos extends CI_Controller {
	...
	public function index() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) {
			...
			$data_usua['titulo']=" de Contratos";
			$data_usua['origen']="Contratos";
			$data_usua['contenido']='conceptos/index';
			$data_usua['entrada_js']='_js/conceptos.js';
	...
	public function listar_tabla() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) {
			...
			if(!$this->input->is_ajax_request()) {
				...
				echo listar_conceptos_tabla('WEB');
			}
		}
	}
	...
	public function guardar() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) 
			...
			if(!$this->input->is_ajax_request()) {
				...
				$query = $this->general_model->insert('conceptos_contratost', $registro);
				if($query >= 1)
					echo '1';
				else {
	...
	public function pdf() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) 
			...
			$query = $this->general_model->consulta_personalizada($campos, 'conceptos_contratost a', '', '', 0, 0);
			...
			$pdf->Cell(70,5,utf8_decode("NOMBRE"),'LTRB',0,'C',$fill);
			$pdf->Cell(10,5,utf8_decode("PREFIJO"),'LTRB',0,'C',$fill);
	        ...
	        foreach ($encabezados as $row) {
                ...
                $pdf->Cell(70,5,utf8_decode($row->Nombre),'LTRB',0,'C',$fill);                
                $pdf->Cell(70,5,utf8_decode($row->Prefijo),'LTRB',0,'C',$fill);
	        ...
	        $pdf->Output('I', "Listado_conceptos.pdf");
	...
	public function excel() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) 
			...
			$filename = "Listado_conceptos.xls";
		    header ("Content-Disposition: attachment; filename=".$filename ); 
			header ("Content-Type: application/vnd.ms-excel");
			
			$this->load->helper('funciones_tabla');
			
		    echo utf8_decode('<table border="1"><tr><th colspan="7">LISTADO CONCEPTOS DE CONTRATOS</th></tr></table><br>');
