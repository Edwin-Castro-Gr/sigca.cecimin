...
class A_eps extends CI_Controller {
	...
	public function index() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) {
			...
			$data_usua['titulo']="Entidades Pagadoras";
			$data_usua['origen']="Administración";
			$data_usua['contenido']='eps/index';
			$data_usua['entrada_js']='_js/eps.js';
	...
	public function guardar() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) 
			...
			if(!$this->input->is_ajax_request()) {
				...
				$query = $this->general_model->insert('eps', $registro);
				if($query >= 1)
					echo '1';
				else {
					echo '<div class="alert alert-danger"><i class="fa fa-ban"></i><strong>¡Error!</strong><br>';
					switch($query) {
						case "1062": echo "La Entidad Pagadora ingresada, ya se encuentra registrada; Por favor verifique los datos!"; break;
	...
	public function pdf() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) 
	        ...
	        $pdf->hoja = 'LETTER';
	        $pdf->SetTitle("SIGCA - Listado de Entidades Pagadoras", true);
	        $pdf->SetLeftMargin(7);
            ...
            $pdf->Cell(0,2,utf8_decode('LISTADO GENERAL DE ENTIDADES PAGADORAS'), 0, 0, 'C', false);
            $pdf->Ln(6);
            ...
            $campos =' e.id_eps AS "Id", e.nombre AS "Nombre", CASE WHEN e.estado="1" THEN "Activo" ELSE "Inactivo" END AS "Estado" ';
			$query = $this->general_model->consulta_personalizada($campos, 'eps e', '', '', 0, 0);
	        ...
	        $pdf->Output('I', "Listado_EntidadesPagadoras.pdf");
	...
	public function excel() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) 
			...
			$filename = "Listado_EntidadesPagadoras.xls";
		    header ("Content-Disposition: attachment; filename=".$filename ); 
			header ("Content-Type: application/vnd.ms-excel");
			
			$this->load->helper('funciones_tabla');
			
		    echo utf8_decode('<table border="1"><tr><th colspan="7">LISTADO GENERAL DE ENTIDADES PAGADORAS</th></tr></table><br>');

		    echo '<table border="1">';
            echo utf8_decode(listar_eps_tabla('EXCEL')); 
            echo '</table>';			
		}//-Valida Inicio de Session
	}
	...
	public function inactivar() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) 
			...
			if(!$this->input->is_ajax_request()) {
				...
				$query = $this->general_model->update('eps', 'id_eps', $this->input->post('idreg'), $registro);
				if($query=="OK")
					echo '1';
				else {
	...
	public function modificar() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) 
			...
			$query=$this->general_model->select_where('id_eps, codigo, nombre, estado', 'eps', array('id_eps' => $id) );
			$row = $query->row_array();
				
			$arr['eps'] = array('id_eps'=>$row['id_eps'], 'codigo'=>$row['codigo'],'nombre'=>$row['nombre'],'estado'=>$row['estado']);
	...
	public function actualizar() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) 
			...
			if(!$this->input->is_ajax_request()) {
				...
				$query = $this->general_model->update('eps', 'id_eps', $this->input->post('idregistro'), $registro);
				if($query=="OK")
					echo '1';
				else {
					echo '<div class="alert alert-danger"><i class="fa fa-ban"></i><strong>¡Error!</strong><br>';
	...
	public function ver_registro() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) {
			...
			if(!$this->input->is_ajax_request()) {
				...
				$campos = ' e.id_eps AS "Id", e.codigo AS "Código",e.nombre AS "Nombre", CASE WHEN e.estado="1" THEN "Activo" ELSE "Inactivo" END AS "Estado" ';
      
		    	$query = $this->general_model->consulta_personalizada($campos, 'eps e', ' e.id_eps = "'.$idreg.'" ', '', 0, 0);
