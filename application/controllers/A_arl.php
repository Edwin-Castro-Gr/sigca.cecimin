...
class A_arl extends CI_Controller {
	...
	public function index() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) {
			...
			$data_usua['titulo']="Administradoras de Riesgos Laborales";
			$data_usua['origen']="Administración";
			$data_usua['contenido']='arl/index';
			$data_usua['entrada_js']='_js/arl.js';
	...
	public function guardar() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) 
			...
			if(!$this->input->is_ajax_request()) {
				...
				$query = $this->general_model->insert('arl', $registro);
				if($query >= 1)
					echo '1';
				else {
					echo '<div class="alert alert-danger"><i class="fa fa-ban"></i><strong>¡Error!</strong><br>';
					switch($query) {
						case "1062": echo "La Administradora de Riesgos Laborales ingresada, ya se encuentra registrada; Por favor verifique los datos!"; break;
	...
	public function pdf() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) 
	        ...
	        $pdf->hoja = 'LETTER';
	        $pdf->SetTitle("SIGCA - Listado de Administradoras de Riesgos Laborales", true);
	        $pdf->SetLeftMargin(7);
            ...
            $pdf->Cell(0,0,utf8_decode('LISTADO ADMINISTRADORAS DE RIESGOS LABORALES'), 0, 0, 'C', false);
            $pdf->Ln(8);
			...
			$query = $this->general_model->consulta_personalizada($campos, 'arl a', '', '', 0, 0);
	...
	public function excel() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) 
			...
			$filename = "Listado_Listado_ARL.xls";
		    header ("Content-Disposition: attachment; filename=".$filename ); 
			header ("Content-Type: application/vnd.ms-excel");
			
			$this->load->helper('funciones_tabla');
			
		    echo utf8_decode('<table border="1"><tr><th colspan="7">LISTADO ADMINISTRADORAS DE RIESGOS LABORALES</th></tr></table><br>');

		    echo '<table border="1">';
            echo utf8_decode(listar_arl_tabla('EXCEL')); 
            echo '</table>';			
		}//-Valida Inicio de Session
	}
	...
	public function inactivar() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) 
			...
			if(!$this->input->is_ajax_request()) {
				...
				$query = $this->general_model->update('arl', 'id_arl', $this->input->post('idreg'), $registro);
				if($query=="OK")
					echo '1';
				else {
	...
	public function modificar() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) 
			...
			$query=$this->general_model->select_where('id_arl, nombre, estado', 'arl', array('id_arl' => $id) );
			$row = $query->row_array();
				
			$arr['arl'] = array('id_arl'=>$row['id_arl'], 'nombre'=>$row['nombre'],'estado'=>$row['estado']);
	...
	public function actualizar() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) 
			...
			if(!$this->input->is_ajax_request()) {
				...
				$query = $this->general_model->update('arl', 'id_arl', $this->input->post('idregistro'), $registro);
				if($query=="OK")
					echo '1';
				else {
					echo '<div class="alert alert-danger"><i class="fa fa-ban"></i><strong>¡Error!</strong><br>';
	...
	public function ver_registro() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) {
			...
			if(!$this->input->is_ajax_request()) {
		    	...
		    	$query = $this->general_model->consulta_personalizada($campos, 'arl a', ' a.id_arl = "'.$idreg.'" ', '', 0, 0);
