...
class M_servicios extends CI_Controller {
	...
	public function index() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) {
			...
			$data_usua['titulo']="Requerimientos Mantenimiento";
			$data_usua['origen']="Mantenimientos";
			$data_usua['contenido']='m_servicios/index';
			$data_usua['entrada_js']='_js/m_servicios.js';
	...
	public function listar_tabla() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) {
			...
			if(!$this->input->is_ajax_request()) {
				...
				echo listar_serviciosMto_tabla('WEB');
			}
		}
	}
	...
	public function guardar() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) 
			...
			if(!$this->input->is_ajax_request()) {
				...
				$query = $this->general_model->insert('mantenimientos_servicios', $registro);
				if($query >= 1)
					echo '1';
				else {
	...
	public function pdf() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) 
	        ...
	        $pdf->hoja = 'LETTER';
	        $pdf->SetTitle("SIGCA - Listado de Requerimientos de Mantenimientos", true);
	        $pdf->SetLeftMargin(7);
            ...
            $pdf->Cell(0,0,utf8_decode('LISTADO GENERAL DE REQUERIMIENTOS '), 0, 0, 'C', false);
            $pdf->Ln(8);
			...
			$query = $this->general_model->consulta_personalizada($campos, 'mantenimientos_servicios s', '', '', 0, 0);
	        ...
	        $pdf->Output('I', "Listado_requerimientos.pdf");
	...
	public function excel() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) 
			...
			$filename = "Listado_requerimientos.xls";
		    header ("Content-Disposition: attachment; filename=".$filename ); 
			header ("Content-Type: application/vnd.ms-excel");
			
			$this->load->helper('funciones_tabla');
			
		    echo utf8_decode('<table border="1"><tr><th colspan="7">LISTADO GENERAL DE REQUERIMIENTOS</th></tr></table><br>');

		    echo '<table border="1">';
            echo utf8_decode(listar_serviciosMto_tabla('EXCEL')); 
            echo '</table>';			
		}//-Valida Inicio de Session
	}
	...
	public function inactivar() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) 
			...
			if(!$this->input->is_ajax_request()) {
				...
				$query = $this->general_model->update('mantenimientos_servicios', 'id_servicio', $this->input->post('idreg'), $registro);
				if($query=="OK")
					echo '1';
				else {
	...
	public function modificar() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) 
			...
			$query=$this->general_model->select_where('id_servicio, nombre, estado', 'mantenimientos_servicios', array('id_servicio' => $id) );
			$row = $query->row_array();
				
			$arr['serviciom'] = array('id_servicio'=>$row['id_servicio'], 'nombre'=>$row['nombre'],'estado'=>$row['estado']);
	...
	public function actualizar() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) 
			...
			if(!$this->input->is_ajax_request()) {
				...
				$query = $this->general_model->update('mantenimientos_servicios', 'id_servicio', $this->input->post('idregistro'), $registro);
				if($query=="OK")
					echo '1';
				else {
					echo '<div class="alert alert-danger"><i class="fa fa-ban"></i><strong>¡Error!</strong><br>';
					switch($query) {
						case "1062": echo "El Requerimiento ingresado, ya se encuentra registrado; Por favor verifique los datos!"; break;
						default: echo "Error: ".$query." => ".$this->db->_error_message(); break;	
					}
					echo '</div>';
				}
	...
	public function ver_registro() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) {
			...
			if(!$this->input->is_ajax_request()) {
				...
				$campos = ' aa.id_servicio AS "Id", aa.nombre AS "Nombre", CASE WHEN aa.estado="1" THEN "Activo" ELSE "Inactivo" END AS "Estado" ';
      
		    	$query = $this->general_model->consulta_personalizada($campos, 'mantenimientos_servicios aa', ' aa.id_servicio = "'.$idreg.'" ', '', 0, 0);
