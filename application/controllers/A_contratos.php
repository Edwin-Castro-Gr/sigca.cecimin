...
class A_contratos extends CI_Controller {
	...
	public function modificar($id) {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) {
			...
			$campos='c.id_ingreso AS "IngresoP", c.id_tipocontrato AS "Tipo", c.id_funcionario AS "Funcionario", e.cedula AS "Cedula", c.id_cargo AS "Cargo", c.id_centrocosto AS "Centro costos", c.id_area AS "Departamento", c.jefe_inmediato AS "Jefe", c.fecha_inicio AS "FechaInicio", c.fecha_final AS "FechaFinal", c.prorroga AS "Prorroga", c.estado AS "Estado", c.id_usuario AS "UsuarioCreo"';

			$query = $this->general_model->consulta_personalizada($campos,'contratos c INNER JOIN empleados e ON c.id_funcionario = e.id_empleado', 'c.id_contrato ="'.$id.'" ', '', 0, 0);
	...
	public function guardar() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) 
			...
			if(!$this->input->is_ajax_request()) {
				...
				$contfiletemp = date('Ymd').$this->session->userdata('C_id_usuario');
				...
				$query2 = $this->general_model->update('contratos_fileContratos', 'id_contrato', $contfiletemp, $registro2);
			...
			if(!$this->input->is_ajax_request()) {
				...
				if($query =="OK"){
					...
					if ($this->input->post('idactulizaranexos')){
	                    ...
	                    // PROCESAMIENTO CORREGIDO DE ARCHIVOS
	                    foreach ($_FILES as $inputName => $fileArray) {
	                        $id_check_contrato = explode('_', $inputName);
	                        $fieldId = end($id_check_contrato);
	                        ...
	                        if (is_array($fileArray['name'])) {
	                            ...
	                            for ($i = 0; $i < count($fileArray['name']); $i++) {
	                                ...
	                                $this->processUpdateFile($currentFile, $rutag, $fieldId, $idcontrato, $fecha, $idingresop, $mensage);
	                            }
	                            
	                        } else {
	                            // Procesar archivo único
	                            $this->processUpdateFile($fileArray, $rutag, $fieldId, $idcontrato, $fecha, $idingresop, $mensage);
	                        }  
	                    }
	                }					
					echo '1';
	...
	// Función auxiliar para procesar archivos en actualizar
	private function processUpdateFile($fileData, $ruta, $fieldId, $contratoId, $fecha, $ingresoP, &$mensage) {
	    if ($fileData['error'] == UPLOAD_ERR_OK) {
	        $NombreOriginal = $fileData['name'];
	        $temporal = $fileData['tmp_name'];
	        ...
	        if (move_uploaded_file($temporal, $Destino)) {
	            ...
	            if($ingresoP == '' || $ingresoP == 0 || $ingresoP == 00){
	                $registro1 = array(
	                    'id_contrato'=>$contratoId,
	                    'archivo'=>$Destino,
	                    'id_checklist_contratos'=>$fieldId,
	                ...
	                $this->general_model->insert('contratos_anexos', $registro1);
	            } else {
	                $registro1 = array(
	                    'id_ingresop' => $ingresoP,
	                    'id_checklist_contratos' => $fieldId,
	                    'archivo' => $Destino,
	                    'fecha_ini_vigencia'=>$this->input->post('fecha_inicio_'.$fieldId),
	                    'fecha_fin_vigencia'=>$this->input->post('fecha_final_'.$fieldId),
	...
	public function guardar_documento()
	{
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) {
			...
			if(!$this->input->is_ajax_request()) {
				...
				$fecha = date('Y-m-d H:i:s');
				$contrato = $this->input->post('idreg_contrato');
				$tem_contrato = $contrato;
				if($contrato == ''){
					$tem_contrato = date("Ymd").$this->session->userdata('C_id_usuario');
				};

				$dir =$this->input->post('cedulaempleadoC');	
				$id_usuario = $this->session->userdata('C_id_usuario');
				$filename ='';
				...
				if ($this->upload->do_upload('archivoCont')){					 							
					$data = array('upload_data' => $this->upload->data());
					$filename = $rutag.$data['upload_data']['file_name'];	
					$registro=array(
						'id_contrato'=>$tem_contrato,	
						'nombre_archivo' =>	$this->input->post('nombre_archivo'),	 					
						'ruta'=> $filename,
						'fecha_documento'=>$this->input->post('fecha_doc'),	
						'fecha_fvigencia'=>$this->input->post('fecha_vigencia'),	
						'fecha_registro'=>$fecha,
						'usuario_temp' =>$this->session->userdata('C_id_usuario'),							
						'estado_doc'=>'0'   
					);
					$query = $this->general_model->insert('contratos_fileContratos', $registro);				
				}else{
					$query ="error_file";
				}
				if($query >=1){
	...
	public function cargar_filecontratos() 
	{
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) {
			redirect(base_url());
		} else {
			if(!$this->input->is_ajax_request()) {
				...
				$campos1 ='id_archivoc AS "Id",nombre_archivo AS "Nombre", ruta AS "Archivo", fecha_documento AS "Fecha_Inicio", fecha_fvigencia AS "Fecha_Final"';
			    $query1 = $this->general_model->consulta_personalizada($campos1, 'contratos_fileContratos', 'id_contrato='.$tem_contrato.'', '', 0, 0);
		    	
		        foreach ($query1->result_array() as $row)
			    {
			    	$accordion .='<tr><td width="35%"><div class="form-group row " id="div_archivo'.$row['Id'].'">
	                          	<div class="col-sm-4 col-form-label text-sm-left pr-0">'.
	                            	form_label(''.$row['Nombre'],'archivo_'.$row['Id'].'', array('class'=>'mb-0')).'
	                          	</div></td>';
			    	$accordion .='<td width="10%"><div class="col-sm-1" id="archivo_'.$row['Id'].'">
                   					'.anchor(base_url().$row['Archivo'], '<i class="fa fa-file-pdf"></i>', array('classid'=>'btn btn-circle btn-outline-danger','style'=>'width: 30px; height: 30px; padding: 2px 0px;font-size: 18px;','target'=>'_blank')).'

                  				  </div></td>';	
              		$accordion .='<td width="40%">
	              					<div class="form-group row d-flex justify-content-between">
			                          	<div class="col-sm-6">
				                          	<span class="">'.
				                            	form_label('Fecha Inicio:','fecha_inicio_'.$row['Id'], array('class'=>'mb-0 class="col-sm-12 col-form-label text-sm-right pr-0"')). $row['Fecha_Inicio'].'
				                          	</span>
			                          	</div>
			                          	
			                          	<div class="col-sm-6">
			                          		<span class="">'.
				                            	form_label('Fecha Final:','fecha_final_'.$row['Id'], array('class'=>'mb-0 class="col-sm-12 col-form-label text-sm-right pr-0"')).' '.$row['Fecha_Final'].'
				                          	</span>
			                          	</div>
			                          </div>
		                          	</td>
	...
	public function cargar_documentosContratos() 
	{
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) {
			redirect(base_url());
		} else {
			if(!$this->input->is_ajax_request()) {
				...
				$campos1 ='id_archivoc AS "Id",nombre_archivo AS "Nombre", ruta AS "Archivo", fecha_documento AS "Fecha_Inicio", fecha_fvigencia AS "Fecha_Final"';
			    $query1 = $this->general_model->consulta_personalizada($campos1, 'contratos_fileContratos', 'id_contrato='.$id_contrato.'', '', 0, 0);
