...
class Administracion_med extends CI_Controller {
	...
	public function index()	
	{
		...
		$this->load->view('medicamentos/index');
	}
	...
	public function reporte()
	{
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) {
			...
			$this->load->helper('funciones_select');
			$data_usua['titulo']="Citas Medicamentos No Oncologicos";
			$data_usua['origen']="Administración";
			$data_usua['contenido']='medicamentos/reporte';
			$data_usua['entrada_js']='_js/medicamentos.js';
	...
	public function listar_tabla() 
	{
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) {
			...
			if(!$this->input->is_ajax_request()) {
			    ...
			    $query = $this->general_model->consulta_personalizada($campos, 'contactenos', '', 'Id', 0, 0);
	...
	public function excel() 
	{
		...
		redirect(base_url());
			...
			$query = $this->general_model->consulta_personalizada($campos, 'contactenos c INNER JOIN servicios s ON c.servicio = s.id_servicio', '', 'c.id_contacto', 0, 0);
			...
			{
				$count++;
				$tabla .= '<tr><td>'.$count.'</td><td>'.$row1['Motivo'].'</td><td>'.$row1['Contacto'].'</td><td>'.$row1['Cedula'].'</td><td>'.$row1['Email'].'</td><td>'.$row1['Teléfono'].'</td><td>'.$row1['Direccion'].'</td><td>'.$row1['Direccion'].'</td><td>'.$row1['Servicio'].'</td><td>'.$row1['Mensaje'].'</td><td>'.$row1['Fecha_hecho'].'</td><td>'.$row1['Hora_hecho'].'</td><td>'.$row1['Accion de Mejora'].'</td><td>'.$row1['Observaciones'].'</td><td>'.$row1['Fecha_registro'].'</td><td>'.$row1['Estado'].'</td></tr>';    					
			}						
	...
	public function gestion($id)
	{
		if (!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="") {
			...
			$data_usua['origen']="Administración";
			$data_usua['contenido']='administracion_med/gestion';
			$data_usua['entrada_js']='_js/medicamentos.js';
			$data_usua['librerias_css']='<!-- DataTables -->
			<link rel="stylesheet" type="text/css"  href="'.base_url('plugins/datatables.net-bs4@1.10.24/css/dataTables.bootstrap4.min.css').'">			
			<link rel="stylesheet" type="text/css" href="'.base_url('plugins/datatables.net-buttons-bs4@1.7.0/css/buttons.bootstrap4.min.css').'">
