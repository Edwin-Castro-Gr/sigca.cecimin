...
class C_documentosc extends CI_Controller {
	...
	public function index() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) {
			...
			$data_usua['titulo']="Documentos Contratos";
			$data_usua['origen']="Documentos";
			$data_usua['contenido']='cdocumentos/index';
			$data_usua['entrada_js']='_js/documentosC.js';
	...
	public function listar_tabla() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) {
			...
			if(!$this->input->is_ajax_request()) {
				...
				echo listar_documentosC_tabla('WEB');
			}
		}
	}
	...
	public function guardar() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) 
			...
			if(!$this->input->is_ajax_request()) {
				...
				$query = $this->general_model->insert('listado_documentos', $registro);
				if($query >= 1)
					echo '1';
				else {
	...
	public function pdf() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) 
	        ...
	        $pdf->hoja = 'LETTER';
	        $pdf->SetTitle("SIGCA - Listado de Documentos Contratos", true);
	        $pdf->SetLeftMargin(7);
            ...
            $pdf->Cell(0,0,utf8_decode('LISTADO GENERAL DE DOCUMENTOS CONTRATOS'), 0, 0, 'C', false);
            $pdf->Ln(8);
			...
			$query = $this->general_model->consulta_personalizada($campos, 'listado_documentos aa', '', '', 0, 0);
	        ...
	        $pdf->Output('I', "Listado_DocumentosC.pdf");
	...
	public function excel() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) 
			...
			$filename = "Listado_DocumentosContratos.xls";
		    header ("Content-Disposition: attachment; filename=".$filename ); 
			header ("Content-Type: application/vnd.ms-excel");
			
			$this->load->helper('funciones_tabla');
			
		    echo utf8_decode('<table border="1"><tr><th colspan="7">LISTADO GENERAL DE DOCUMENTOS CONTRATOS </th></tr></table><br>');

		    echo '<table border="1">';
            echo utf8_decode(listar_documentosC_tabla('EXCEL')); 
            echo '</table>';			
		}//-Valida Inicio de Session
	}
	...
	public function inactivar() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) 
			...
			if(!$this->input->is_ajax_request()) {
				...
				$query = $this->general_model->update('listado_documentos', 'id_listado', $this->input->post('idreg'), $registro);
				if($query=="OK")
					echo '1';
				else {
	...
	public function modificar() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) 
			...
			$query=$this->general_model->select_where('id_listado, nombre, estado', 'listado_documentos', array('id_listado' => $id) );
			$row = $query->row_array();
				
			$arr['documentos'] = array('id_listado'=>$row['id_listado'], 'nombre'=>$row['nombre'],'estado'=>$row['estado']);
	...
	public function actualizar() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) 
			...
			if(!$this->input->is_ajax_request()) {
				...
				$query = $this->general_model->update('listado_documentos', 'id_listado', $this->input->post('idregistro'), $registro);
				if($query=="OK")
					echo '1';
				else {
					echo '<div class="alert alert-danger"><i class="fa fa-ban"></i><strong>¡Error!</strong><br>';
	...
	public function ver_registro() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) {
			...
			if(!$this->input->is_ajax_request()) {
		    	...
		    	$query = $this->general_model->consulta_personalizada($campos, 'listado_documentos aa', ' aa.id_listado = "'.$idreg.'" ', '', 0, 0);
