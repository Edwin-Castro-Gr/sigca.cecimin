...
class C_materiales extends CI_Controller {
	...
	public function index() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) {
			...
			$data_usua['titulo']="Materiales Qx";
			$data_usua['origen']="Cirugía";
			$data_usua['contenido']='materiales/index';
			$data_usua['entrada_js']='_js/materiales.js';
	...
	public function listar_tabla() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) {
			...
			if(!$this->input->is_ajax_request()) {
				...
				echo listar_materialesqx_tabla('WEB');
			}
		}
	}
	...
	public function guardar() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) 
			...
			if(!$this->input->is_ajax_request()) {
				...
				//echo "ingreso";
				$registro=array(
					'id_grupo'=>$this->input->post('grupos_materialesqx'), 
					'nombre_material'=>$this->input->post('nombre'),
					'fecha_registro'=>date('Y-m-d H:i:s'), 
					'id_usuario'=>$this->session->userdata('C_id_usuario'), 
					'estado'=>'1'
				);

				$query = $this->general_model->insert('materiales_qx', $registro);
				if($query >= 1)
					echo '1';
				else {
					echo '<div class="alert alert-danger"><i class="fa fa-ban"></i><strong>¡Error!</strong><br>';
					switch($query) {
						case "1062": echo "El Material ingresado, ya se encuentra registrado; Por favor verifique los datos!"; break;
	...
	public function pdf() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) 
	        ...
	        $pdf->hoja = 'LETTER';
	        $pdf->SetTitle("SIGCA - Listado de Materiales", true);
	        $pdf->SetLeftMargin(7);
            ...
            $pdf->Cell(0,0,utf8_decode('LISTADO GENERAL DE MATERIALES   '), 0, 0, 'C', false);
            $pdf->Ln(10);
			...
			$query = $this->general_model->consulta_personalizada($campos, 'materiales_qx m LEFT JOIN materiales_grupos p ON m.id_grupo = p.id_grupo', '', '', 0, 0);
			...
			$pdf->Cell(100,5,utf8_decode("GRUPO"),'LTRB',0,'C',$fill);
	        ...
	        foreach ($encabezados as $row) {
                ...
                $pdf->Cell(100,5,utf8_decode($row->Procedimiento),'LTRB',0,'C',$fill);
	        ...
	        $pdf->Output('I', "Listado_Materiales.pdf");
	...
	public function excel() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) 
			...
			$filename = "Listado_Materiales.xls";
		    header ("Content-Disposition: attachment; filename=".$filename ); 
			header ("Content-Type: application/vnd.ms-excel");
			
			$this->load->helper('funciones_tabla');
			
		    echo utf8_decode('<table border="1"><tr><th colspan="7">LISTADO GENERAL DE MATERIALES QX</th></tr></table><br>');

		    echo '<table border="1">';
            echo utf8_decode(listar_materialesqx_tabla('EXCEL')); 
            echo '</table>';			
		}//-Valida Inicio de Session
	}
	...
	public function inactivar() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) 
			...
			if(!$this->input->is_ajax_request()) {
				...
				$query = $this->general_model->update('materiales_qx', 'id_material', $this->input->post('idreg'), $registro);
				if($query=="OK")
					echo '1';
				else {
	...
	public function modificar() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) 
			...
			$query=$this->general_model->select_where('id_material, id_grupo, nombre_material, estado', 'materiales_qx', array('id_material' => $id) );
			$row = $query->row_array();
				
			$arr['materiales_qx'] = array('id_material'=>$row['id_material'], 'id_grupo'=>$row['id_grupo'], 'nombre_material'=>$row['nombre_material'], 'estado'=>$row['estado']);
	...
	public function actualizar() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) 
			...
			if(!$this->input->is_ajax_request()) {
				...
				$query = $this->general_model->update('materiales_qx', 'id_material', $this->input->post('idregistro'), $registro);
				if($query=="OK")
					echo '1';
				else {
					echo '<div class="alert alert-danger"><i class="fa fa-ban"></i><strong>¡Error!</strong><br>';
	...
	public function ver_registro() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) {
			...
			if(!$this->input->is_ajax_request()) {
		    	...
		    	$query = $this->general_model->consulta_personalizada($campos, 'materiales_qx m LEFT JOIN materiales_grupos p ON m.id_grupo = p.id_grupo', ' m.id_material= "'.$idreg.'" ', '', 0, 0);
