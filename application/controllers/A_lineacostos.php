...
class A_lineacostos extends CI_Controller {
	...
	public function index() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) {
			...
			$data_usua['titulo']="Linea de Costos";
			$data_usua['origen']="Administración";
			$data_usua['contenido']='lineacostos/index';
			$data_usua['entrada_js']='_js/lineacostos.js';
	...
	public function listar_tabla() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) {
			...
			if(!$this->input->is_ajax_request()) {
				...
				echo listar_lineacostos_tabla('WEB');
			}
		}
	}
	...
	public function guardar() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) 
			...
			if(!$this->input->is_ajax_request()) {
				...
				$query = $this->general_model->insert('linea_costos', $registro);
				if($query >= 1)
					echo '1';
				else {
					echo '<div class="alert alert-danger"><i class="fa fa-ban"></i><strong>¡Error!</strong><br>';
					switch($query) {
						case "1062": echo "La Linea de Costos ingresada, ya se encuentra registrado; Por favor verifique los datos!"; break;
	...
	public function pdf() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) 
	        ...
	        $pdf->hoja = 'LETTER';
	        $pdf->SetTitle("SIGCA - Listado de Lineas de Costos", true);
	        $pdf->SetLeftMargin(7);
            ...
            $pdf->Cell(0,0,utf8_decode('LISTADO GENERAL DE LINEAS DE COSTOS   '), 0, 0, 'C', false);
            $pdf->Ln(8);
			...
			$query = $this->general_model->consulta_personalizada($campos, 'linea_costos lc ', '', '', 0, 0);
	        ...
	        $pdf->Output('I', "Listado_LineasCostos.pdf");
	...
	public function excel() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) 
		    ...
		    echo utf8_decode('<table border="1"><tr><th colspan="7">LISTADO GENERAL DE LINEAS DE COSTOS</th></tr></table><br>');

		    echo '<table border="1">';
            echo utf8_decode(listar_lineacostos_tabla('EXCEL')); 
            echo '</table>';			
		}//-Valida Inicio de Session
	}
	...
	public function inactivar() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) 
			...
			if(!$this->input->is_ajax_request()) {
				...
				$query = $this->general_model->update('linea_costos', 'id_linea_costos', $this->input->post('idreg'), $registro);
				if($query=="OK")
					echo '1';
				else {
	...
	public function modificar() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) 
			...
			$query=$this->general_model->select_where('id_linea_costos, nombre, estado', 'linea_costos', array('id_linea_costos' => $id) );
			$row = $query->row_array();
				
			$arr['linea_costos'] = array('id_linea_costos'=>$row['id_linea_costos'], 'nombre'=>$row['nombre'],'estado'=>$row['estado']);
	...
	public function actualizar() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) 
			...
			if(!$this->input->is_ajax_request()) {
				...
				$query = $this->general_model->update('linea_costos', 'id_linea_costos', $this->input->post('idregistro'), $registro);
				if($query=="OK")
					echo '1';
				else {
					echo '<div class="alert alert-danger"><i class="fa fa-ban"></i><strong>¡Error!</strong><br>';
	...
	public function ver_registro() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) {
			...
			if(!$this->input->is_ajax_request()) {
		    	...
		    	$query = $this->general_model->consulta_personalizada($campos, 'linea_costos lc ', ' lc.id_linea_costos = "'.$idreg.'" ', '', 0, 0);
