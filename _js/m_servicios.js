$(function () {
	...
	function cargar_listado() {
	    ...
	    $.post("/m_servicios/listar_tabla",{}, function(data_carg){
	      //alert(data_carg);
  	...
  	$(document).on("click", function(event){
	    ...
	    if(datos[0] == "btneditar") {
	      idreg = datos[1];
	      $('#newModalLabel').html('Modificar servicio Mantenimiento');
	      
	      $.post("/m_servicios/modificar",{idreg: ""+idreg+""}, function(data_preg){
        
	        $('#idregistro').val(data_preg['serviciom'].id_servicio);
	        
	        $("#nombre").val(data_preg['serviciom'].nombre);

	        $('#div_estado').css("display", "flex");
	        $("#estado").val(data_preg['serviciom'].estado);
	        $("#estado").change();
	      });
	      ...
	      //jQuery(function(){
	        ...
	        Swal.fire({
              ...
              customClass: {
	            ...
	            if (result.value) {
	            	$.post("/m_servicios/inactivar",{idreg: ""+id_reg+""}, function(data_form){
	            		//alert(data_form);
	            		if(data_form=="1") {
			              Swal.fire({
			                title: 'Inactivado!',
	    ...
	    if(datos[0] == "btndetalle") {
	      ...
	      $.post("/m_servicios/ver_registro",{idreg: ""+idreg+""}, function(data_carg){
	        //alert(data_carg);
	    ...
	    if(dato == "btn_guardar") {
	      ...
	      if(ban==1) { 	
	        ...
	        $.post("/m_servicios/guardar", datos_form , function(data_form){
	          Swal.close();
	          if(data_form=="1") {
	            //jQuery(function(){
	              Swal.fire({
	  	...
	  	if(dato == "btn_nuevo_Servicio") {
	  		$('#newModalLabel').html('Nuevo Servicio Mantenimiento');
	      ...
	      if(ban==1) { 	
	        ...
	        var datos_form = $("#form_guardar").serialize();
	        $.post("/m_servicios/actualizar", datos_form , function(data_form){
	          Swal.close();
	          if(data_form=="1") {
	            //jQuery(function(){
	              Swal.fire({
	  	...
	  	if(dato == "btn_pdf") {
	  		window.open('/m_servicios/pdf','_blank');
	  	}

	  	if(dato == "btn_excel") {
	  		window.open('/m_servicios/excel','_blank');
	  	}
  	});
