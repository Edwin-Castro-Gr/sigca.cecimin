$(function () {
	...
	function cargar_listado() {
	    Swal.fire({ 
	      ...
	      text: "Cargando lista de ARL ",
	      showConfirmButton: false 
	    });
	    
	    $.post("/a_arl/listar_tabla",{}, function(data_carg){
	      //alert(data_carg);
  	...
  	$(document).on("click", function(event){
	    ...
	    if(datos[0] == "btneditar") {
	      idreg = datos[1];
	      $('#newModalLabel').html('Modificar ARL');
	      
	      $.post("/a_arl/modificar",{idreg: ""+idreg+""}, function(data_preg){
        
	        $('#idregistro').val(data_preg['arl'].id_arl);
	        $("#nombre").val(data_preg['arl'].nombre);

	        $('#div_estado').css("display", "flex");
	        $("#estado").val(data_preg['arl'].estado);
	        $("#estado").change();
	      });
	      ...
	      //jQuery(function(){
	        ...
	        Swal.fire({
              ...
              customClass: {
	            ...
	            if (result.value) {
	            	$.post("/a_arl/inactivar",{idreg: ""+id_reg+""}, function(data_form){
	            		//alert(data_form);
	            		if(data_form=="1") {
			              Swal.fire({
			                title: 'Inactivado!',
	    ...
	    if(datos[0] == "btndetalle") {
	      ...
	      $.post("/a_arl/ver_registro",{idreg: ""+idreg+""}, function(data_carg){
	        //alert(data_carg);
	    ...
	    if(dato == "btn_guardar") {
	      ...
	      if(ban==1) { 	
	        ...
	        $.post("/a_arl/guardar", datos_form , function(data_form){
	          Swal.close();
	          if(data_form=="1") {
	            //jQuery(function(){
	              Swal.fire({
	  	...
	  	if(dato == "btn_nuevo_arl") {
	  		$('#newModalLabel').html('Nueva ARL');
	      ...
	      if(ban==1) { 	
	        ...
	        var datos_form = $("#form_guardar").serialize();
	        $.post("/a_arl/actualizar", datos_form , function(data_form){
	          Swal.close();
	          if(data_form=="1") {
	            //jQuery(function(){
	              Swal.fire({
	  	...
	  	if(dato == "btn_pdf") {
	  		window.open('/a_arl/pdf','_blank');
	  	}

	  	if(dato == "btn_excel") {
	  		window.open('/a_arl/excel','_blank');
	  	}
  	});
