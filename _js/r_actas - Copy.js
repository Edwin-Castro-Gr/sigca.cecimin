$(function () {
    ...
    if ($('#opc_pag').val() == "ingreso") {
        ...
        $('#btn_Actualizar').click(function () {
            if($('#usuarioResponsable').val() == $('#usuarioActual').val() || $('#empleados_proyecta').val() == $('#usuarioActual').val()){
                var CkModificar = document.getElementById('usuarioResponsable');
                alert(CkModificar);
                if($('#modificar_acta').is(':checked')){
                    ...
                    if($('#cantPart').val()!= '0'){
                        ...
                        if(firmado){
                            ...
                            if($('#cantObser').val() != '0'){
                                Swal.fire("¡Atención!", 'Todos los participantes no han firmado', "question");
                            }else{                             
                                guardar_firma();   
                            }
                        }                        
                ...
                for (var i = 1; i <= cantPart; i++) {
                    var idParticipante = $('#idusuarioFir' + i).val();
                ...
                if (usuarioYaFirmo) {
                    Swal.fire("¡Atención!", 'el usuario ya firmo, No hay cambios que guardar', "warning");
                }else{ 
                    if($('#usuariofirma').val() == "00"){ 
                        if($('#cantObser').val() == '0'){
                            Swal.fire("¡Atención!", 'No ha firmado, Desea registrar observaciones', "question");
                        }else{                        
    ...
    $(document).on("click", function (event) {
        ...
        if(datos[0] =="eliminarObs"){
           ...
           id_acta = datos[2];
            ...
            $.post("/r_actas/cargar_observaciones",{idActa: ""+ id_acta+""}, function(data_obser){
               $('#pos-obser').html(data_obser);
            });
    ...
    function inactivar_elementos(resp){
        if(resp){
            ...
            $('#Ncomite').attr('readonly',false);
            $('#otroNC').prop('readonly',false);
            $('#motivo').prop('readonly',false);
            $('#empleados_responsable').attr('readonly','').trigger('change');
            ...
            $('#empleados_responsable').prop('readonly', true).trigger('change');
