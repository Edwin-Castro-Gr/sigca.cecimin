$(function () {
  ...
  if ($('#opc_pag').val() == "listado") {
    ...
    if(($("#tiposcontratos_ingresop").val()==2)||($("#tiposcontratos_ingresop").val()==4)||($("#tiposcontratos_ingresop").val()==5)||($("#tiposcontratos_ingresop").val()==8)||($("#tiposcontratos_ingresop").val()==7)){
    ...
    $("input[name=actulizaranexos]").change((e)=>{ 
      ...
      if((e.target.checked)==true){
        $.post("/c_ingresop/cargar_anexos",{id_ingreso: ""+$('#idregistro').val()+"", tipo_cont: ""+tipo_cont+""}, function(data_carg){
          //alert(data_carg);
    ...
    $('#cedula_empleado').attr("readonly", true);
    $('#tiposcontratos_ingresop').attr("readonly", true);
    $('#cargos_ingresop').attr("readonly", true);
    $('#centroscostos_ingresop').attr("readonly", true);
    $('#lineacostos_ingresop').attr("readonly", true);
    $('#areas_ingresop').attr("readonly", true);
    $('#empleados_jefeinm').attr("readonly", true);
    $('#fechainicio').attr("readonly", true);
    $('#fechafinal').attr("readonly", true);
  ...
  $(document).on("click", function (event) {
    ...
    if (datos[0] == "btnrevisar") {
      idreg = datos[1];
      idusuario = datos[2];
      window.open('/c_ingresop/revisar/'+idreg,'_parent');
    }
    ...
    if (dato == "btnGuardarRevision") {  
      guardarRevision_registro();
    }
    ...
    if(e.keyCode == 13 || e.keyCode == 9) {
      ...
      $.post("/c_ingresop/cargar_empleado",{emple: ""+cedula+""}, function(data_emple){
        if((data_emple['empleado'].id_empleado)){
          ...
          $('#correo_empleado').val(data_emple['empleado'].email_empleado);                         
          $('#btn_agregar_empleado').css("display", "none");             
  ...
  function guardarRevision_registro() { 
    Swal.close();    
    Swal.fire({   
      title: "Por favor espere!",   
      text: "Actualizando la información.", 
    ...
    var formData = new FormData(document.getElementById("form_revisar"));

    $.ajax({
      url: "/c_ingreso/guardarRevisar",
      type: "POST",
      dataType: "html",
      data: formData,
      cache: false,
