$(function () {
	...
	function cargar_listado() {
	    Swal.fire({ 
	      ...
	      text: "Cargando lista de politicas.",
	      showConfirmButton: false 
	    });
	    
	    $.post("/a_politicas/listar_tabla",{}, function(data_carg){
	      //alert(data_carg);
  	...
  	$(document).on("click", function(event){
	    ...
	    if(datos[0] == "btneditar") {
	      idreg = datos[1];
	      $('#newModalLabel').html('Modificar politica');
	      
	      $.post("/a_politicas/modificar",{idreg: ""+idreg+""}, function(data_preg){
        
	        $('#idregistro').val(data_preg['politicas'].id_politica);

	        $("#nombre").val(data_preg['politicas'].nombre);
	        $("#descripcion").val(data_preg['politicas'].descripcion);
	        	        
	        $('#div_estado').css("display", "flex");
	        $("#estado").val(data_preg['politicas'].estado);
	        $("#estado").change();
	      });
	      ...
	      //jQuery(function(){
	        ...
	        Swal.fire({
              ...
              customClass: {
	            ...
	            if (result.value) {
	            	$.post("/a_politicas/inactivar",{idreg: ""+id_reg+""}, function(data_form){
	            		//alert(data_form);
	            		if(data_form=="1") {
			              Swal.fire({
			                title: 'Inactivado!',
	    ...
	    if(datos[0] == "btndetalle") {
	      ...
	      $.post("/a_politicas/ver_registro",{idreg: ""+idreg+""}, function(data_carg){
	        //alert(data_carg);
	    ...
	    if(dato == "btn_guardar") {
	      ...
	      if( $('#descripcion').val()=="" ){
	        $('#descripcion').addClass("brc-danger");
	        texto=texto+"* La descripcion de la politica es obligatoria!";
	        ban=1;
	      }
	      ...
	      if(ban==1) { 	
	        ...
	        $.post("/a_politicas/guardar", datos_form , function(data_form){
	          Swal.close();
	          if(data_form=="1") {
	            //jQuery(function(){
	              Swal.fire({
	  	...
	  	if(dato == "btn_nueva_politica") {
	  		
	  		$('#newModalLabel').html('Nueva politica');
	      ...
	      if(ban==1) { 	
	        ...
	        var datos_form = $("#form_guardar").serialize();
	        $.post("/a_politicas/actualizar", datos_form , function(data_form){
	          Swal.close();
	          if(data_form=="1") {
	            //jQuery(function(){
	              Swal.fire({
	  	...
	  	if(dato == "btn_pdf") {
	  		window.open('/a_politicas/pdf','_blank');
	  	}

	  	if(dato == "btn_excel") {
	  		window.open('/a_politicas/excel','_blank');
	  	}
  	});
