...
$(function () {
	...
	if ($('#opc_pag').val() == "reportes") {
	    ...
	    $.post("/linea_etica/listar_tabla", {}, function (data_carg) {
	      //alert(data_carg);
		...
		$.post("/linea_etica/cargar_origen", {}, function (data_carg) {
		 	$('#tdenuncia').html(data_carg);
		});
	////////////####### inicio smartwizard ########///////////////
	// Leave step event is used for validating the forms
        ...
        $("#sumunistra_info").on("change",function(){
        	if($("#sumunistra_info").val() == "0"){
        		$("#container_denunciante").css('display', 'none');
        	}else{
        		$("#container_denunciante").css('display', 'flex');
        	}        	
        });

         $("#sumunistra_evid").on("change",function(){
        	if($("#sumunistra_evid").val()== "0"){
        		$("#div_archivov").css('display', 'none');
        	}else{
        		$("#div_archivov").css('display', 'flex');
        	}        	
        });
 	...
 	$(document).on("click", function (event) {
      ...
      var dato = event.target.id;
		...
		if (dato == "btn_sucess_modal") {
			...
			// alert(fecha[0]); 
			// alert(fecha[1]); 
        	window.open('/linea_etica/excel/'+idmes, '_blank'); 						
      }

      if(datos[0] == "btndetalle") {
	      idreg = datos[1];
	      
	      $.post('/linea_etica/ver_registro',{idreg: ""+idreg+""}, function(data_carg){
	   ...
	   if(datos[0] =="btngestionar"){
	    	id_cont = datos[1];				
	    	window.open('/linea_etica/gestion/' +id_cont,'_parent');
	   }
