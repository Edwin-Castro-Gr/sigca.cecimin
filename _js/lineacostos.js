$(function () {
	...
	function cargar_listado() {
	    Swal.fire({ 
	      ...
	      text: "Cargando lista de Linea de Costos.",
	      showConfirmButton: false 
	    });
	    
	    $.post("/a_lineacostos/listar_tabla",{}, function(data_carg){
	      //alert(data_carg);
  	...
  	$(document).on("click", function(event){
	    ...
	    if(datos[0] == "btneditar") {
	      ...
	      $.post("/a_lineacostos/modificar",{idreg: ""+idreg+""}, function(data_preg){
        
	        $('#idregistro').val(data_preg['linea_costos'].id_lineacosto);
	        $("#nombre").val(data_preg['linea_costos'].nombre);
	        $('#div_estado').css("display", "flex");
	        $("#estado").val(data_preg['linea_costos'].estado);
	        $("#estado").change();
	      });
	      ...
	      //jQuery(function(){
	        ...
	        Swal.fire({
              ...
              customClass: {
	            ...
	            if (result.value) {
	            	$.post("/a_lineacostos/inactivar",{idreg: ""+id_reg+""}, function(data_form){
	            		//alert(data_form);
	            		if(data_form=="1") {
			              Swal.fire({
			                title: 'Inactivado!',
	    ...
	    if(datos[0] == "btndetalle") {
	      ...
	      $.post("/a_lineacostos/ver_registro",{idreg: ""+idreg+""}, function(data_carg){
	        //alert(data_carg);
	    ...
	    if(dato == "btn_guardar") {
	      ...
	      if(ban==1) { 	
	        ...
	        $.post("/a_lineacostos/guardar", datos_form , function(data_form){
	          Swal.close();
	          if(data_form=="1") {
	            //jQuery(function(){
	              Swal.fire({
	  	...
	  	if(dato == "btn_nueva_lineacostos") {
	  		$('#newModalLabel').html('Nueva Linea de Costos');
	      ...
	      if(ban==1) { 	
	        ...
	        var datos_form = $("#form_guardar").serialize();
	        $.post("/a_lineacostos/actualizar", datos_form , function(data_form){
	          Swal.close();
	          if(data_form=="1") {
	            //jQuery(function(){
	              Swal.fire({
	  	...
	  	if(dato == "btn_pdf") {
	  		window.open('/a_lineacostos/pdf','_blank');
	  	}

	  	if(dato == "btn_excel") {
	  		window.open('/a_lineacostos/excel','_blank');
	  	}
  	});
