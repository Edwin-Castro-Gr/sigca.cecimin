$(function () {
	...
	function cargar_listado() {
	    Swal.fire({ 
	      ...
	      text: "Cargando lista de Entidades Pagadoras.",
	      showConfirmButton: false 
	    });
	    
	    $.post("/a_eps/listar_tabla",{}, function(data_carg){
	      //alert(data_carg);
  	...
  	$(document).on("click", function(event){
	    ...
	    if(datos[0] == "btneditar") {
	      idreg = datos[1];
	      $('#newModalLabel').html('Modificar Entidad Pagadora');
	      
	      $.post("/a_eps/modificar",{idreg: ""+idreg+""}, function(data_preg){
        
	        $('#idregistro').val(data_preg['eps'].id_eps);
	        $("#codigo").val(data_preg['eps'].codigo);
	        $("#nombre").val(data_preg['eps'].nombre);

	        $('#div_estado').css("display", "flex");
	        $("#estado").val(data_preg['eps'].estado);
	        $("#estado").change();
	      });
	      ...
	      //jQuery(function(){
	        ...
	        Swal.fire({
              ...
              customClass: {
	            ...
	            if (result.value) {
	            	$.post("/a_eps/inactivar",{idreg: ""+id_reg+""}, function(data_form){
	            		//alert(data_form);
	            		if(data_form=="1") {
			              Swal.fire({
			                title: 'Inactivado!',
	    ...
	    if(datos[0] == "btndetalle") {
	      ...
	      $.post("/a_eps/ver_registro",{idreg: ""+idreg+""}, function(data_carg){
	        //alert(data_carg);
	    ...
	    if(dato == "btn_guardar") {
	      ...
	      if(ban==1) { 	
	        ...
	        $.post("/a_eps/guardar", datos_form , function(data_form){
	          Swal.close();
	          if(data_form=="1") {
	            //jQuery(function(){
	              Swal.fire({
	  	...
	  	if(dato == "btn_nuevo_eps") {
	  		$('#newModalLabel').html('Nueva Entidad Pagadora');
	      ...
	      if(ban==1) { 	
	        ...
	        var datos_form = $("#form_guardar").serialize();
	        $.post("/a_eps/actualizar", datos_form , function(data_form){
	          Swal.close();
	          if(data_form=="1") {
	            //jQuery(function(){
	              Swal.fire({
	  	...
	  	if(dato == "btn_pdf") {
	  		window.open('/a_eps/pdf','_blank');
	  	}

	  	if(dato == "btn_excel") {
	  		window.open('/a_eps/excel','_blank');
	  	}
  	});
