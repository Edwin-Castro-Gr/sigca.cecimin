$(function () {
    ...
    $(document).on("click", function (event) {
        ...
        if (dato == "btn_cargar_excel") {
            var archivo = $('#archivo_evidencia')[0].files[0];
            ...
            if (extension != 'xlsx' && extension != 'xls' && extension != 'csv') {
                Swal.fire("¡Atención!", "Por favor seleccione un archivo válido (xlsx, xls o csv)", "warning");
                return false;
            }

            Swal.fire({
                title: "Por favor espere!",
                text: "Cargando y procesando el archivo Excel...",
                showConfirmButton: false,
            ...
            formData.append('id_tarifa', $('#idregistro').val());

            $.ajax({
                url: "/c_tarifas/cargar_excel_tarifas",
                type: "POST",
                data: formData,
                cache: false,
                contentType: false,
                    ...
                    try {
                        ...
                        if (res.status == "success") {
                            Swal.fire({
                                ...
                                // Opcional: recargar alguna tabla o actualizar la vista
                                if (res.data && res.data.length > 0) {
                                    mostrarDatosCargados(res.data);
                                }
                            });
                        } else {
    ...
    function mostrarDatosCargados(datos) {
        // Esta función es opcional - puedes crear una tabla o mostrar los datos cargados
        var html = '<div class="table-responsive"><table class="table table-bordered table-striped">';
        html += '<thead><tr><th>CÓDIGO</th><th>DESCRIPCIÓN</th><th>CLASE TARIFA</th><th>TARIFA</th><th>UVR</th><th>PLAN</th></tr></thead><tbody>';
        ...
        // Mostrar en un modal o en algún div de la página
        Swal.fire({
