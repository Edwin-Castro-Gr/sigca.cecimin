$(function () {
	...
	function cargar_listado() {
	    Swal.fire({ 
	      ...
	      text: "Cargando lista de conceptos ",
	      showConfirmButton: false 
	    });
	    
	    $.post("/d_conceptos/listar_tabla",{}, function(data_carg){
	      //alert(data_carg);
  	...
  	$(document).on("click", function(event){
	    ...
	    if(datos[0] == "btneditar") {
	      ...
	      $.post("/d_conceptos/modificar",{idreg: ""+idreg+""}, function(data_preg){
        
	        $('#idregistro').val(data_preg['conceptos'].id_arl);
	        $("#nombre").val(data_preg['conceptos'].nombre);
	        $("#prefijo").val(data_preg['conceptos'].prefijo);

	        $('#div_estado').css("display", "flex");
	        $("#estado").val(data_preg['conceptos'].estado);
	        $("#estado").change();
	      });
	      ...
	      //jQuery(function(){
	        ...
	        Swal.fire({
              ...
              customClass: {
	            ...
	            if (result.value) {
	            	$.post("/d_conceptos/inactivar",{idreg: ""+id_reg+""}, function(data_form){
	            		//alert(data_form);
	            		if(data_form=="1") {
			              Swal.fire({
			                title: 'Inactivado!',
	    ...
	    if(datos[0] == "btndetalle") {
	      ...
	      $.post("/d_conceptos/ver_registro",{idreg: ""+idreg+""}, function(data_carg){
	        //alert(data_carg);
	    ...
	    if(dato == "btn_guardar") {
	      ...
	      if(ban==1) { 	
	        ...
	        $.post("/d_conceptos/guardar", datos_form , function(data_form){
	          Swal.close();
	          if(data_form=="1") {
	            //jQuery(function(){
	              Swal.fire({
	  	...
	  	if(dato == "btn_nuevo_concepto") {
	  		$('#newModalLabel').html('Nuevo Concepto');
	      ...
	      if(ban==1) { 	
	        ...
	        var datos_form = $("#form_guardar").serialize();
	        $.post("/d_conceptos/actualizar", datos_form , function(data_form){
	          Swal.close();
	          if(data_form=="1") {
	            //jQuery(function(){
	              Swal.fire({
	  	...
	  	if(dato == "btn_pdf") {
	  		window.open('/d_conceptos/pdf','_blank');
	  	}

	  	if(dato == "btn_excel") {
	  		window.open('/d_conceptos/excel','_blank');
	  	}
  	});
