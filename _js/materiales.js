$(function () {
	...
	function cargar_listado() {
	    Swal.fire({ 
	      ...
	      text: "Cargando lista de Materiales.",
	      showConfirmButton: false 
	    });
	    
	    $.post("/c_materiales/listar_tabla",{}, function(data_carg){
	      //alert(data_carg);
  	...
  	$(document).on("click", function(event){
	    ...
	    if(datos[0] == "btneditar") {
	      idreg = datos[1];
	      $('#newModalLabel').html('Modificar Materiales Qx');
	      
	      $.post("/c_materiales/modificar",{idreg: ""+idreg+""}, function(data_preg){
        
	        $('#idregistro').val(data_preg['materiales_qx'].id_material);
	        $("#grupos_materialesqx").val(data_preg['materiales_qx'].id_grupo);
	        $("#grupos_materialesqx").change();

	        $("#nombre").val(data_preg['materiales_qx'].nombre_material);
	        $('#div_estado').css("display", "flex");
	        $("#estado").val(data_preg['materiales_qx'].estado);
	        $("#estado").change();
	      });
	      ...
	      //jQuery(function(){
	        ...
	        Swal.fire({
              ...
              customClass: {
	            ...
	            if (result.value) {
	            	$.post("/c_materiales/inactivar",{idreg: ""+id_reg+""}, function(data_form){
	            		//alert(data_form);
	            		if(data_form=="1") {
			              Swal.fire({
			                title: 'Inactivado!',
	    ...
	    if(datos[0] == "btndetalle") {
	      ...
	      $.post("/c_materiales/ver_registro",{idreg: ""+idreg+""}, function(data_carg){
	        //alert(data_carg);
	    ...
	    if(dato == "btn_guardar") {
	      ...
	      if( $('#grupos_materialesqx').val()=="" ){
	        $('#grupos_materialesqx').addClass("brc-danger");
	        texto=texto+"* El Procedimiento es obligatorio!";
	        ban=1;
	      }
	      ...
	      if(ban==1) { 	
	        ...
	        $.post("/c_materiales/guardar", datos_form , function(data_form){
	          Swal.close();
	          if(data_form=="1") {
	            //jQuery(function(){
	              Swal.fire({
	  	...
	  	if(dato == "btn_nuevo_material") {
	  		$('#newModalLabel').html('Nuevo Material');
	      ...
	      if(ban==1) { 	
	        ...
	        var datos_form = $("#form_guardar").serialize();
	        $.post("/c_materiales/actualizar", datos_form , function(data_form){
	          Swal.close();
	          if(data_form=="1") {
	            //jQuery(function(){
	              Swal.fire({
	  	...
	  	if(dato == "btn_pdf") {
	  		window.open('/c_materiales/pdf','_blank');
	  	}

	  	if(dato == "btn_excel") {
	  		window.open('/c_materiales/excel','_blank');
	  	}
  	});
