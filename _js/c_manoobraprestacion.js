$(function () {
	...
	function cargar_listado() {
	    Swal.fire({ 
	      ...
	      text: "Cargando lista de Costos Mano Obra Prestacion de Servicios.",
	      showConfirmButton: false 
	    });
	    
	    $.post("/cc_manoobraprestacion/listar_tabla",{}, function(data_carg){
	      //alert(data_carg);
  	...
  	$(document).on("click", function(event){
	    ...
	    if(datos[0] == "btneditar") {
	      ...
	      $.post("/cc_manoobraprestacion/modificar",{idreg: ""+idreg+""}, function(data_preg){
	      ...
	      //jQuery(function(){
	        ...
	        Swal.fire({
              ...
              customClass: {
	            ...
	            if (result.value) {
	            	$.post("/cc_manoobraprestacion/inactivar",{idreg: ""+id_reg+""}, function(data_form){
	            		//alert(data_form);
	            		if(data_form=="1") {
			              Swal.fire({
			                title: 'Inactivado!',
	    ...
	    if(datos[0] == "btndetalle") {
	      ...
	      $.post("/cc_manoobraprestacion/ver_registro",{idreg: ""+idreg+""}, function(data_carg){
	        //alert(data_carg);
	    ...
	    if(dato == "btn_guardar") {
	        ...
	        $.post("/cc_manoobraprestacion/guardar", datos_form , function(data_form){
	          	Swal.close();
	          	if(data_form=="1") {
	            //jQuery(function(){
	              	Swal.fire({
  	...
  	if(dato == "btn_nueva_manoobrapresta") {
	  	...
	  	if(dato == "btn_actualizar") {
		    ...
		    if(ban==1) { 	
		        ...
		        var datos_form = $("#form_guardar").serialize();
		        $.post("/cc_manoobraprestacion/actualizar", datos_form , function(data_form){
		          	Swal.close();
		          	if(data_form=="1") {
		            //jQuery(function(){
		              	Swal.fire({
	  	...
	  	if(dato == "btn_pdf") {
	  		window.open('/cc_manoobraprestacion/pdf','_blank');
	  	}

	  	if(dato == "btn_excel") {
	  		window.open('/cc_manoobraprestacion/excel','_blank');
	  	}
  	});
