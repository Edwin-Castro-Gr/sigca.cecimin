$(function () {
	...
	function cargar_listado() {
	    Swal.fire({ 
	      ...
	      text: "Cargando lista de Cargos.",
	      showConfirmButton: false 
	    });
	    
	    $.post("/a_configuraciones/listar_tabla",{}, function(data_carg){
	      //alert(data_carg);
  	...
  	$(document).on("click", function(event){
	    ...
	    if(datos[0] == "btneditar") {
	      idreg = datos[1];
	      $('#newModalLabel').html('Modificar Cargos');
	      
	      $.post("/a_configuraciones/modificar",{idreg: ""+idreg+""}, function(data_preg){
        
	        $('#idregistro').val(data_preg['cargos'].id_cargo);

	        $("#nombre").val(data_preg['cargos'].nombre);
	        $("#titulo").val(data_preg['cargos'].titulo);
	        $("#naturaleza").val(data_preg['cargos'].naturaleza);
	        
	        $('#div_estado').css("display", "flex");
	        $("#estado").val(data_preg['cargos'].estado);
	        $("#estado").change();
	      });
	      ...
	      //jQuery(function(){
	        ...
	        Swal.fire({
              ...
              customClass: {
	            ...
	            if (result.value) {
	            	$.post("/a_configuraciones/inactivar",{idreg: ""+id_reg+""}, function(data_form){
	            		//alert(data_form);
	            		if(data_form=="1") {
			              Swal.fire({
			                title: 'Inactivado!',
	    ...
	    if(datos[0] == "btndetalle") {
	      ...
	      $.post("/a_configuraciones/ver_registro",{idreg: ""+idreg+""}, function(data_carg){
	        //alert(data_carg);
	    ...
	    if(dato == "btn_guardar") {
	      ...
	      if( $('#titulo').val()=="" ){
	        $('#titulo').addClass("brc-danger");
	        texto=texto+"* El titulo es obligatorio!";
	        ban=1;
	      }
	      ...
	      if(ban==1) { 	
	        ...
	        $.post("/a_configuraciones/guardar", datos_form , function(data_form){
	          Swal.close();
	          if(data_form=="1") {
	            //jQuery(function(){
	              Swal.fire({
	  	...
	  	if(dato == "btn_nuevo_cargo") {
	  		
	  		$('#newModalLabel').html('Nuevo Cargo');
	      ...
	      if( $('#titulo').val()=="" ){
	        $('#titulo').addClass("brc-danger");
	        texto=texto+"* El titulo del cargo es obligatorio!";
	        ban=1;
	      }
	      ...
	      if(ban==1) { 	
	        ...
	        var datos_form = $("#form_guardar").serialize();
	        $.post("/a_configuraciones/actualizar", datos_form , function(data_form){
	          Swal.close();
	          if(data_form=="1") {
	            //jQuery(function(){
	              Swal.fire({
	  	...
	  	if(dato == "btn_pdf") {
	  		window.open('/a_configuraciones/pdf','_blank');
	  	}

	  	if(dato == "btn_excel") {
	  		window.open('/a_configuraciones/excel','_blank');
	  	}
  	});
